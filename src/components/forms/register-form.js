...
const RegisterForm = memo(({ isSubmitting, errors, values, setValidationClass, wasError }) => {
    const history = useHistory();

    return (
        <Form className="form">
            {wasError && <ErrorMsg text={'Something wrong!'}/>}
            ...
            <div className="input__container">
                <Field type="password" name="password_confirmation" placeholder="Repeat Password" className={setValidationClass(errors.password_confirmation, values.password_confirmation)} />
                <ErrorMessage name="password_confirmation" component="div" className="error" />
            </div>
            <button type="submit" disabled={isSubmitting} className="btn-submit">
                Sign Up
            </button>
            <div className="redirect__btn" onClick={() => history.push('/login')}>
                Sign In
            </div>
        </Form>
    );
...
export default withFormik({ name: '', email: '', password: '', password_confirmation: '' })(RegisterForm);
